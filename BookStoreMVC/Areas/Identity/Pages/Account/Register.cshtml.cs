...
namespace BookStoreMVC.Areas.Identity.Pages.Account
{
    public class RegisterModel : PageModel
    {
        ...
        private BookStoreMVCUser CreateUser()
        {
            try
            {
                return Activator.CreateInstance<BookStoreMVCUser>();
            }
            catch
            {
                throw new InvalidOperationException($"Can't create an instance of '{nameof(BookStoreMVCUser)}'. " +
                    $"Ensure that '{nameof(BookStoreMVCUser)}' is not an abstract class and has a parameterless constructor, or alternatively " +
                    $"override the register page in /Areas/Identity/Pages/Account/Register.cshtml");
            }
        }

        private IUserEmailStore<BookStoreMVCUser> GetEmailStore()
        {
            if (!_userManager.SupportsUserEmail)
            {
                throw new NotSupportedException("The default UI requires a user store with email support.");
            }
            return (IUserEmailStore<BookStoreMVCUser>)_userStore;
        }
    }
}
